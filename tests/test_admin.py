...
def test_admin_stats_and_audit_logs(client: TestClient, db_session):
    # ثبت‌نام عمومی هرگز نقش admin صادر نمی‌کند؛ bootstrap ادمین باید صریح باشد.
    res_reg = client.post(
        "/api/auth/register",
    ...
    admin_user = db_session.query(User).filter_by(username_key="superadmin").one()
