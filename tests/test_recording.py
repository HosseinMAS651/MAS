...
def test_automatic_recording_lifecycle(client: TestClient):
    ...
    res_c1 = client.post(
        ...
        files={"chunk": ("chunk_1.bin", io.BytesIO(chunk1), "application/octet-stream")},
    )
    assert res_c1.status_code == 200

    res_c2 = client.post(
        f"/api/rooms/{room_id}/recording/chunk",
        data={"session_id": session_id, "seq": 2},
        files={"chunk": ("chunk_2.bin", io.BytesIO(chunk2), "application/octet-stream")},
    )
