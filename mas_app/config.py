...
class Settings(BaseSettings):
    ...
    # ── مدیر ─────────────────────────────────────────────────
    bootstrap_admin_username: str = ""  # deprecated; never grants admin on public registration
    ...
    def _validate_environment_specific(self) -> Settings:
        ...
        # Render و اکثر پلتفرم‌ها DATABASE_URL را با postgres:// یا postgresql:// می‌دهند.
        # پروژه از psycopg3 استفاده می‌کند؛ باید به postgresql+psycopg:// تبدیل شود
        # تا SQLAlchemy به جای psycopg2، درایور درست را بارگذاری کند.
        ...
        if self.cookie_samesite == "none" and not self.cookie_secure:
            raise SettingsError("MAS_COOKIE_SAMESITE=none فقط همراه MAS_COOKIE_SECURE فعال مجاز است.")
        return self
