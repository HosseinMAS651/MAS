...
def _snapshot_etag(snapshot: dict, room: Room) -> str:
    payload = {
        ...
        "limit_ms": snapshot["limit_ms"],
        "speakers": [
            {"id": s.id, "order": s.order_index, "finished": s.is_finished,
             "elapsed": s.timer.elapsed_ms if s.timer else 0,
             "overtime": s.timer.overtime_ms if s.timer else 0,
...
def get_timer_state(
    ...
    etag = _snapshot_etag(snapshot, room)
...
async def timer_action(
    ...
    snapshot = await timer_service.handle_action(
        session, room, payload.action, speaker_id=payload.speaker_id, expected_version=payload.expected_version
    )
    state_resp = _format_timer_state(room, snapshot)
    return {"ok": True, "state": state_resp}
