...
from ..core.validation import validate_timezone
...
class AuthService:
    ...
    def register(
        ...
        *,
        ...
        # ثبت‌نام عمومی هرگز نباید امتیاز Admin بدهد. مدیر اولیه با ابزار bootstrap جداگانه ساخته می‌شود.
        role = "user"

        validate_timezone(timezone or "Asia/Tehran")
        ...
        try:
            ...
            # A concurrent request can legitimately race the username pre-check.
            # Only translate an actual username-uniqueness violation; unrelated
            # schema/data errors must reach the global DB error handler instead of
            # being misreported as "username already exists".
            ...
            is_username_conflict = (
                constraint_name in {
                    ...
                    "uq_users_username_key_legacy",
                    "ix_users_username_key",
                }
                or ("unique constraint" in orig_text and "username_key" in orig_text)
                or ("unique constraint failed" in orig_text and "users.username_key" in orig_text)
            )
            if is_username_conflict:
                raise ValidationAppError("این نام کاربری قبلاً ثبت شده است.") from exc
            raise
    ...
    def login(
        ...
        *,
        ...
        # Rate limit is scoped to both IP and normalized username so one attacker
        # cannot lock a real user out from a different network.
        if ip:
            RateLimiter.check_and_increment(
                session,
                f"login:ip:{ip}",
        ...
        # Reset only the current caller's rate-limit buckets.
        if ip:
            RateLimiter.reset(session, f"login:ip:{ip}")
            RateLimiter.reset(session, f"login:userip:{u_key}:{ip}")
        else:
