...
from ..db.models import AuditLog, Room, SpeechFile, User
...
class AdminService:
    ...
    def list_users(session: Session, *, limit: int = 50, offset: int = 0) -> list[dict]:
        ...
        results = []
        for u in users:
            rooms_count = session.execute(
                select(func.count(Room.id)).where(Room.owner_id == u.id)
            ).scalar() or 0
            results.append(
                {
                    "id": u.id,
    ...
    def list_rooms(session: Session, *, limit: int = 50, offset: int = 0) -> list[dict]:
        rooms = session.execute(
            select(Room).order_by(Room.id.desc()).offset(offset).limit(limit)
        ).scalars().all()
