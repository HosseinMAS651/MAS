...
class SecurityManager:
    ...
    def extract_client_ip(self, request_headers: dict[str, str], peer_ip: str | None) -> str:
        ...
        # X-Forwarded-For به شکل client, proxy1, proxy2,... وارد می‌شود.
        # وقتی N پراکسی مورداعتماد داریم، آی‌پی N خانه مانده از انتها، کلاینت واقعی است.
        # اگر زنجیره کوتاه‌تر بود، اولین مقدار محافظه‌کارانه‌ترین انتخاب است.
        if len(parts) > hops:
            candidate = parts[-(hops + 1)]
        else:
            candidate = parts[0]
