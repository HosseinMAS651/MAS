...
def _migrate_legacy_database_if_needed(db: Database, inspector: object) -> None:
    ...
    with db.unit_of_work() as session:
        ...
        # ۴. ارتقای جداول کمکی که ممکن است از نسخهٔ ناقص ساخته شده باشند
        rlb_cols = _cols("rate_limit_buckets")
        if rlb_cols:
            rlb_needed = [
            ...
            missing_rlb = [s for n, s in rlb_needed if n not in rlb_cols]
            if missing_rlb:
                logger.info("ارتقای جدول rate_limit_buckets...")
                _add_columns(session, "rate_limit_buckets", missing_rlb)

        as_cols = _cols("auth_sessions")
        if as_cols:
            as_needed = [
                ("csrf_hash", "ALTER TABLE auth_sessions ADD COLUMN csrf_hash VARCHAR(64) DEFAULT ''"),
                ("token_hash", "ALTER TABLE auth_sessions ADD COLUMN token_hash VARCHAR(64)"),
                ("created_at_ms", "ALTER TABLE auth_sessions ADD COLUMN created_at_ms BIGINT DEFAULT 0"),
                ("expires_at_ms", "ALTER TABLE auth_sessions ADD COLUMN expires_at_ms BIGINT DEFAULT 0"),
                ("last_seen_at_ms", "ALTER TABLE auth_sessions ADD COLUMN last_seen_at_ms BIGINT DEFAULT 0"),
                ("ip", "ALTER TABLE auth_sessions ADD COLUMN ip VARCHAR(64) DEFAULT ''"),
                ("user_agent", "ALTER TABLE auth_sessions ADD COLUMN user_agent VARCHAR(255) DEFAULT ''"),
            ]
            missing_as = [s for n, s in as_needed if n not in as_cols]
            if missing_as:
                logger.info("ارتقای جدول auth_sessions...")
                _add_columns(session, "auth_sessions", missing_as)

        al_cols = _cols("audit_logs")
        if al_cols:
            al_needed = [
                ("actor_username", "ALTER TABLE audit_logs ADD COLUMN actor_username VARCHAR(32) DEFAULT ''"),
                ("action", "ALTER TABLE audit_logs ADD COLUMN action VARCHAR(48) DEFAULT ''"),
                ("severity", "ALTER TABLE audit_logs ADD COLUMN severity VARCHAR(16) DEFAULT 'info'"),
                ("target_type", "ALTER TABLE audit_logs ADD COLUMN target_type VARCHAR(32) DEFAULT ''"),
                ("target_id", "ALTER TABLE audit_logs ADD COLUMN target_id VARCHAR(64) DEFAULT ''"),
                ("detail", "ALTER TABLE audit_logs ADD COLUMN detail TEXT DEFAULT ''"),
                ("ip", "ALTER TABLE audit_logs ADD COLUMN ip VARCHAR(64) DEFAULT ''"),
                ("user_agent", "ALTER TABLE audit_logs ADD COLUMN user_agent VARCHAR(255) DEFAULT ''"),
                ("created_at_ms", "ALTER TABLE audit_logs ADD COLUMN created_at_ms BIGINT DEFAULT 0"),
            ]
            missing_al = [s for n, s in al_needed if n not in al_cols]
            if missing_al:
                logger.info("ارتقای جدول audit_logs...")
                _add_columns(session, "audit_logs", missing_al)

        # ۵. جداول کاملاً جدید در صورت عدم وجود
