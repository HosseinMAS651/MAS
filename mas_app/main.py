...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def integrity_error_handler(request: Request, exc: IntegrityError) -> JSONResponse:
        logger.exception("خطای integrity در %s", request.url.path)
        return JSONResponse(
            status_code=409,
            content={
                "ok": False,
                "error": {
                    "code": "DATA_CONFLICT",
                    "message": "دادهٔ ارسالی با وضعیت فعلی سامانه تداخل دارد. لطفاً دوباره تلاش کنید.",
                    "details": {},
                },
            },
        )
    ...
    async def operational_db_error_handler(request: Request, exc: OperationalError) -> JSONResponse:
        logger.exception("خطای اتصال/عملیات دیتابیس در %s", request.url.path)
        return JSONResponse(
            status_code=503,
            content={
                "ok": False,
    ...
    async def sqlalchemy_error_handler(request: Request, exc: SQLAlchemyError) -> JSONResponse:
        logger.exception("خطای SQL در %s", request.url.path)
        return JSONResponse(
            status_code=500,
            content={
                "ok": False,
                "error": {
                    "code": "DATABASE_ERROR",
                    "message": "خطایی در پردازش دیتابیس رخ داد. لطفاً دوباره تلاش کنید.",
                    "details": {},
                },
            },
        )
