...
export const RoomsListPage: React.FC = () => {
  ...
  const openQrForRoom = async (roomId: number, roomName: string) => {
    try {
      ...
      if (!res.room?.public_token) {
        // فعال‌سازی لینک عمومی در صورت عدم وجود
        const tokenRes = await api.post(`/api/rooms/${roomId}/rotate-public-token`);
        setQrModalRoom({ name: roomName, token: tokenRes.public_token });
      } else {
